...
const getPatternChars = (flt: WordFilter) =>
  Array.from({ length: flt.length ?? 0 }).map(
    (_, i) => flt.pattern[i] ?? null
  ...
  return (
    <div className="font-monospace">
      {chars.map((char, i) => (
        <span
          key={i}
          className={cs("px-0", char !== null && "fw-bold")}
          style={{ width: "30px" }}
        >
          {char ?? "?"}
        </span>
...
export const PatternFilterButtons = (props: {
  filter: WordFilter;
...
export const PatternFilterOffCanvas = (props: Props) => {
  ...
  const setChar = (value: string | null) => {
    ...
    props.onChange([
      ...chars.slice(0, selection),
      value,
      ...chars.slice(selection + 1),
    ]);
  };
